...
while IFS= read -r pack; do
    ...
    if ! grep -Eq "resource[[:space:]]+[[:alnum:]_]+[[:space:]]+'Radius\\.Core/recipePacks@" "$pack"; then
    ...
    unexpected_resources="$(
        grep -En "^[[:space:]]*resource[[:space:]]+[[:alnum:]_]+[[:space:]]+'" "$pack" |
            grep -v "Radius\\.Core/recipePacks@" || true
    )"
    if [[ -n "$unexpected_resources" ]]; then
        echo "Error: Recipe Pack declares resources other than Radius.Core/recipePacks: ${pack#"$REPO_ROOT/"}" >&2
