...
Add an `objectStorage` resource to your application and connect a container to it. One connection named `storage` injects ordinary `CONNECTION_STORAGE_ENDPOINT`, `CONNECTION_STORAGE_ACCOUNTNAME`, and `CONNECTION_STORAGE_CONTAINERNAME` values plus secret-backed `CONNECTION_STORAGE_CONNECTIONSTRING` and `CONNECTION_STORAGE_ACCOUNTKEY`. No second managed-Secret connection is needed. For custom Kubernetes configuration, `store.properties.secrets.name` remains available as the `secretName` for an explicitly authored `secretKeyRef`. See [`test/app.bicep`](test/app.bicep) for a complete example.
