...
| `password` | string | Optional | The resource ID of the `Radius.Security/secrets` resource that holds the broker password under key `password` (set to `<secretResource>.id`). If omitted, the Kubernetes Recipe generates a random password and returns it through the managed secret. |
