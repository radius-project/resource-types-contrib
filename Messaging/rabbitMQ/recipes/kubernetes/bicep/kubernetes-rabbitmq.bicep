...
@description('Random fallback password used when the resource does not provide a password.')
@secure()
...
// `radius`). The password comes from the sensitive resource property when supplied;
// otherwise, the Recipe generates a random value. The effective password is always
// returned through Radius managed secrets and mounted through secretKeyRef.
var username = context.resource.properties.?username ?? 'radius'

var passwordInput = context.resource.properties.?password ?? ''
var usesSuppliedPassword = !empty(passwordInput)
var password = usesSuppliedPassword ? passwordInput : generatedPassword
var credentialsSecretName = '${uniqueName}-credentials'
...
resource brokerCredentials 'core/Secret@v1' = {
  metadata: {
  ...
  spec: {
    ...
    template: {
      metadata: {
        labels: labels
        annotations: {
          // Credential changes update the Pod template so the broker restarts and
          // reads the updated Secret during deployment.
          'radapp.io/password-hash': uniqueString(password)
        }
      }
      spec: {
        ...
        initContainers: [
          {
            ...
            env: [
              ...
              {
                // Sourced from the Recipe-created Kubernetes Secret — never a
...
output result object = {
  resources: [
    '/planes/kubernetes/local/namespaces/${brokerCredentials.metadata.namespace}/providers/core/Secret/${brokerCredentials.metadata.name}'
  ...
  values: {
    // Non-secret connection values. Clients combine these with the password from
    // the managed Radius.Security/secrets resource.
    host: host
    port: port
    username: username
  }
