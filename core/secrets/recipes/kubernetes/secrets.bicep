...
// If secretKind is not set, set to 'generic' secret
var secretKind = context.resource.properties.kind == null ? 'generic' : context.resource.properties.kind
...
var secretType = secretKind == 'certificate-pem' ? 'kubernetes.io/tls' : (secretKind == 'basicAuthentication' ? 'kubernetes.io/basic-auth' : (secretKind == 'sshKey' ? 'kubernetes.io/ssh-auth' : 'Opaque'))
