...
Add a `redisCaches` resource to your application and connect a container to it. On compatible Kubernetes versions, one connection named `redis` injects ordinary `CONNECTION_REDIS_HOST` and `CONNECTION_REDIS_PORT` values plus a secret-backed variable for each key the selected Recipe returns in `result.secrets`: `CONNECTION_REDIS_URL`, and `CONNECTION_REDIS_ACCESSKEY` when the Recipe provides `accessKey`. For custom or backward-compatible Kubernetes configuration, bind the desired key explicitly with `secretKeyRef` using `redis.properties.secrets.name`; [`test/app.bicep`](test/app.bicep) retains this portable explicit wiring.
