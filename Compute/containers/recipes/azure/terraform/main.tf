# ------------------------------------------------------------
# Azure-environment container recipe.
#
# Containers in a Radius application always run on the Kubernetes
# cluster that hosts Radius itself, regardless of which cloud
# provider hosts the application's data services. This recipe is
# therefore intentionally identical to
# Compute/containers/recipes/kubernetes/terraform/main.tf -- it
# exists so the Azure recipe pack can resolve
# Radius.Compute/containers when an Azure environment is in use.
#
# Keep this file in sync with the kubernetes copy.
# ------------------------------------------------------------
