...
| context.resource.properties.host | Service in-cluster DNS name | Read-only. Convenience alias populated only when the resource exposes exactly one Service (single port-exposing container). Lets peers use the ergonomic `<peer>.properties.host` instead of hardcoding a Service name. Omitted when the resource exposes zero or multiple Services; use `hosts` in that case. |