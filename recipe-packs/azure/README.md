...
| `mySqlServerConfigurations` | Server parameters forwarded verbatim to the AVM MySQL flexible server `configurations` array for `Radius.Data/mySqlDatabases`, using the AVM item shape `{ name, source, value }`. Defaults to `[{ name: 'require_secure_transport', value: 'ON' }]`, preserving the flexible server's default so existing deployments keep requiring TLS. Override to disable `require_secure_transport` so `Radius.Data/mySqlDatabases` matches the Kubernetes and AWS Recipes for this Resource Type, whose `sslMode` output is always `disabled` (see [issue #301](https://github.com/radius-project/resource-types-contrib/issues/301)). Optional. |
